  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className=' mt-5'>
          <h3 className=' font-bold mb-3 text-white text-2xl'>{name}</h3>
...
const Works = () => {
  return (
    <>
      ...
      >
        <p className={styles.sectionSubText}>My workss...</p>
        <h2 className={styles.sectionHeadText}>Projects,</h2>
      </motion.div>

      <div className=' w-full flex' >
        <motion.p variants={fadeIn("", "", 0.1, 1)} className=' mt3 text-secondary text-[17px] max-w-3xl leading-[30px] '>
      ...
      <div className=' mt-20 flex flex-wrap gap-7'>
        {projects.map((proj, index) => (
          <ProjectCard key={`proj ${proj}`} {...proj} index={index} />
        ))}
      </div>
    </>
  )
